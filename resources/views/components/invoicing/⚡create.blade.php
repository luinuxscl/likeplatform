...
new #[Layout('layouts.app')] #[Title('New Invoice')] class extends Component {
    ...
    {
        $count = Invoice::forTenant($this->tenant)->count() + 1;
...
<div class="py-6 max-w-3xl">
    <flux:heading size="xl" class="mb-6">{{ __('New Invoice') }}</flux:heading>
    ...
    <form wire:submit="save" class="space-y-6">
        ...
        <div class="flex items-center gap-3">
            ...
            <flux:button wire:navigate href="{{ route('invoicing.index') }}">{{ __('Cancel') }}</flux:button>
        </div>
    </form>
</div>
